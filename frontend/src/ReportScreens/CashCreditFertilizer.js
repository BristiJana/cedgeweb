...
export default function CashCreditFertilizer() {
...
const ShowBranch = (value) => {
      ...
      const districtList = () => {
       ...
       const branch_Id = branchId ? branchId : 0;
        api
          .creditData(acToken,bankId,branch_Id)
          .then((response) => {
          ...
          const menu = (
            credit && credit.web_result ? (
              <Menu onClick={handleDropdownChange}>
                {credit.web_result.map((option) => (
    ...
    const BarChart = () => {
      ...
      const chartData = extractChartData(credit);
      const data = {
