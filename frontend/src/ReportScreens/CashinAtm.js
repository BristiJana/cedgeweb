...
export default function CashinAtm() {
    
    const [district, setDistrict] = useState([]);
    const[Casinatm,setCasinatm]=useState([]);
...
const ShowBranch = (value) => {
      ...
      const districtList = () => {
       ...
       const branch_Id = branchId ? branchId : 0;
        api
          .cashinatmData(acToken,bankId,branch_Id)
          .then((response) => {
            console.log("resultkcc----->" + JSON.stringify(response.data, null, 2));
            
            setCasinatm(response.data);
